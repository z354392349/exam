...
data.czxt = [
    ...
    },
    {
        ...
        normal: [
            ...
            '2.查找 TLB，如果找到页号，则把该页所在的页框号用于形成物理地址。否则(TLB 失效)查找内存页表，从内存页表中找到相应的页表项，读取页所在的页  框号，以形成物理地址。',
            '3.如果所査找的页表项不在 TLB 中，在访问完内存页表后，要把找到的页表项中的页号和页框号写到 TLB 中。如果 TLB 中的条目已满，系统会根据某种策略 (如最近最少使用替换)选择一个 TLB 中的条目，用刚访问的页表项信息替换选中的这个 TLB 条目。'
        ],
        start: 1,
    },
    {
    ...
    },
    {
        ...
        normal: [
            ...
            '为了实现段页式存储管理的地址映射，操作系统为每个进程建立一个段表，为进程的每个段建立一个页表。',
    ...
    },
    {
        ...
        normal: [
            '实现文件存储，有以下几种常用方式：',
            '1.连续分配方式；2.使用磁盘连接表的分配方式；3.使用内存的链接表分配方式；4.i-结点方式。',
        ],
        start: 1,
    },
    {
        ...
        simple: "无",
        normal: [
            '使用两级目录的优点是解决了文件的重名和文件共享问题，查找时间降低。缺点  是增加了系统的存储开销。',
        ],
        start: 1,
    },
    {
        ...
        simple: "无",
        normal: [
            'ASCII 文件由多行正文组成，在某些系统中每行用回车符结束，某些则用换行符结束，而有些系统还同时采用回车符和换行符，如 MS-DOS。各行的长度不必相同。  是增加了系统的存储开销。',
        ],
        start: 1,
    },
    {
    ...
    // 第九页
    {
        ...
        simple: "无",
        normal: [
            '树形目录的优点是便于文件的分类，层次结构清晰，便于管理和保护，解决了重名问题，查找速度加快。缺点是找一个文件按路径名逐层检查, 由于每个文件都放在外存中，多次访问磁盘会影响速度，结构相对复杂。'
        ],
        start: 1,
    },
    {
        ...
        simple: "无",
        normal: [
            '磁盘空间管理是文件系统的重要功能，包括记录空闲磁盘信息、设计文件的存放方式，以及规定文件系统的簇大小等内容。, 由于每个文件都放在外存中，多次访问磁盘会影响速度，结构相对复杂。'
        ],
        start: 1,
    },
    {
        ...
        simple: "无",
        normal: [
            'I/O 软件的总体目标是将软件组织成一种层次结构，低层软件用来屏蔽硬件的具体细节，高层软件则主要是为用户提供一个简洁、规范的界面。, 由于每个文件都放在外存中，多次访问磁盘会影响速度，结构相对复杂。'
        ],
        start: 1,
    },
    {
        ...
        simple: "无",
        normal: [
            'I/O 操作系统的设备管理软件可以实现如下功能：',
            '1.实现 I / O 设备的独立性；2.错误处理；3.异步传输；4.缓冲管理；5.设备的分配和释放；6.实现 I/O 控制方式, 由于每个文件都放在外存中，多次访问磁盘会影响速度，结构相对复杂。'
        ],
        start: 1,
    },
    {
    ...
    },
    {
        ...
        simple: "无",
        normal: [
            '使用轮询控制方式使 CPU 经常处于由于输入/输出而造成的循环测试状态，造成 CPU 的极大浪费，影响整个系统的吞吐量。',
        ],
        start: 1,
    },
    {
    ...
    },
    {
        ...
        simple: "无",
        normal: [
            '在多道程序环境下，利用一道程序来模拟脱机输入时的外围控制机的功能，把低速 I/O 设备上的数据传送到高速输出磁盘上，再利用另一道程序来模拟脱机输出时外围控制机的功能，把数据从磁盘传送到低速输出设备上。这种在联机情况下实现的同时外围操作称为 SPOOLing。'
        ],
        start: 1,
    },
    {
    ...
    {
        title: "简述 SPOOLing 系统的特点有哪些？",
        simple: "无",
        normal: [
            'SPOOLing 系统的特点如下：1.提高了 I/O 速度；2.将独占设备改造为共享设备；  3.实现了虚拟设备功能。'
        ],
        start: 1,
    },
    {
        title: "简述磁盘的访问时间包括哪些部分？",
        simple: "无",
        normal: [
            '磁盘设备在工作时以恒定速率旋转。为了读或写，磁头必须能移动到所要求的磁道上，并等待所要求的扇区的开始位置旋转到磁头下，然后再开始读或写数据。故可把对磁盘的访问时间分成以下 3 部分：1.寻道时间；2.旋转延迟时间；3.传输时间。'
        ],
        start: 1,
    },
    {
