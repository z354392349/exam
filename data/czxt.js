data.czxt = [
    ...
    {
        title: "调用创建新进程的系统调用来创建进程的一般步骤有哪些？ ",
        simple: "无",
        normal: [
            '1.申请空白PCB',
            '2.为新进程分配资源',
    ...
    },
    {
        ...
        simple: " ",
        normal: [
            '将就绪队列分成多个独立队列，根据进程的某些属性，如果需要占用的内存大小、进程优先权和进程类型，进程会被永远分配到一个队列，每个队列有自己的调度算法。'

        ],
        start: 2,
    },
    {
        title: "时间片大小的确定要考虑什么因素？",
        simple: " ",
        normal: [
            '1.系统对响应时间的要求',
            '2.就绪队列中进程的数目',
    ...
    {
        title: "静态优先权可以根据什么来确定？",
        simple: " ",
        normal: [
            '静态优先权通常可以根据进程的类型，进程需要的资源数量和用户的要求来设定。'

        ],
        start: 2,
    },
    {
        title: "请说明短进程优先调度算法的优缺点？ ",
        simple: " ",
        normal: [
            '从就绪队列中选择估计运行时间最短的进程，为该进程分配CPU。(什么事短进程优先调度算法)',
    ...
    },
    {
        ...
        simple: " ",
        normal: [
            '提供必要的调度信息：就绪时间，截止开始时间，完成截止时间，',
            '系统处理能力强、采用抢占式调度机制、具有快速切换机制',
        ],
        start: 2,
    },
    {
        title: "快速切换具有什么能力？ / 填空题",
        simple: " ",
        normal: [
            '快速切换机制具有对外部中断的快速响应能力和快速的进程切换能力',
        ],
        start: 2,
    },
    {
        title: "为了实现系统调度系统可能需要调度程序提供哪些信息？",
        simple: " ",
        normal: [
