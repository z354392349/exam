data.wlyl = [
  ...
  {
    title: "常见的物理介质有哪些？列举不同的物理介质并说明其主要特性。  \/ 第六章",
    simple: "",
    normal: [
      '引导型传输介质',
      '架空明线：传输损耗降低；但易受天气和外界电磁干扰，对外界噪声敏感，带宽有限。',
      '双绞线：可减少相邻导线的电磁干扰。主要用于基带传输，衰落随频率的升高而增大。',
  ...
  {
    title: "数字基带传输系统主要有哪些部分组成？各部分的主要功能是什么？  \/ 第六章",
    simple: "",
    normal: [
  ...
  },
  {
    ...
    simple: "",
    normal: [
      'QAM 调制：正交幅值调制（QAM）也称为幅值相位联合键控（APK），是一种具有高频带利用率，且可以自适应调整调制速率的调制技术。',
  ...
  },
  {
    ...
    simple: "",
    normal: [
      '机械特性：也叫物理特性，指明通信实体间硬件连接接口的机械特点，如接口所用接线器的形状和尺寸、引线数目和排列、固定和锁定装置等。',
      '电气特性：规定了在物理连接上，导线的电气连接及有关电路的特性，一般包括接收器和发送器电路特性的说明、信号的电平、最大传输速率的说明等。',
      '功能特性：指明物理接口各条信号的用途，包括接口信号线功能的规定方法以及接口信号线的功能分类。',
      '规程特性：指明利用接口传输比特流的全过程，以及各项用于传输的事件发生的合法顺序，包括事件的执行顺序好数据传输方式。'
  ...
  {
    title: "为什么在无限局域网中不使用 CSMA/CD 协议而使用 CSMA/CA 协议？ \/ 第七章",
    simple: "",
    normal: [
  ...
  {
    title: "随着移动节点离开基站越来越远，为了保证传送的帧的丢失概率不增加，基站可以采取的两种策略是什么？ \/ 第七章",
    simple: "",
    normal: [
      '增加传输功率、降低传输速率',
    ],
  ...
  {
    title: "如果某主机通过无线网络连接因特网，那么该主机必须是移动的吗？假设一个使用便携计算机的用户带着计算机在室内散步，并且总是通过相同的接入点接入因特网，那么，从网络的角度看，该用户是移动的吗？为什么？  \/ 第七章",
    simple: "",
    normal: [
      '1.不必须是移动的。因为无线网络不一定是移动网络，但移动网络一定是无线网络。移动网络中的移动结点是随时间改变其与网络连接位置的结点，并且一定采用无线通信技术。',
      '2.不是移动的。从网络层的角度看，该用户并没有移动。因为移动结点随时间改变，其与网络连接位置的结点并没有发生变化，都是通过相同的接入点接入因特网。从链路层角度来看该用户也没有移动，因为该用户不论在何处都与同一个接入点相关联。'
    ],
    start: 1,
  },
  {
    title: "永久地址与转交地址之间有什么区别？谁为移动主机指派转交地址？  \/ 第七章",
    simple: "",
    normal: [
      ...
      '该移动结点在它的（外部代理的）网络中具有给定的 COA。该 COA 的作用是将数据报通过外部代理“重新路由选择”到移动结点'
    ],
    start: 1,
  },
  {
    title: "假设 IEEE 802.11 的 RTS 帧和 CTS 帧与标准的 DATA 帧和 ACK 一样长，使用 RTS 帧和 CTS 帧会有好处吗？为什么？  \/ 第七章",
    simple: "",
    normal: [
      '没有好处。因为设计 RTS 和 CTS 采用短帧就是要用短帧来代替长数据碰撞，即使产生了短帧的碰撞也不至于太浪费，从而提高了协议利用率。',
    ],
    start: 1,
  },
  {
    title: "蓝牙中的主设备和 IEEE 802.11 网络中的基站之间有什么区别？  \/ 第七章",
    simple: "",
    normal: [
      '蓝牙网络以小范围、低功率和低成本运行。',
      'IEEE802.11 是一个大功率、中等范围、高速率的“接入”技术。'
    ],
    start: 1,
  },
  ...
  {
    title: "简述 IEEE 802.11 中无线主机与 AP 的关联过程。 \/ 第七章",
    simple: "",
    normal: [
      '1.每个 AP 周期性地发送信标帧，用户的无线站点通过扫描信道，获得正在发送信标帧的 AP。',
      '2.通过信标帧得到可用的 AP 后，选择其中一个进行关联；',
      '3.在探测到多个可选的 AP 后，选择其中一个 AP 与之关联；',
      '4.然后无线主机向 AP 发送一个关联请求帧，该 AP 以一个关联响应帧进行响应；',
      '5.一旦与一个 AP 相关联，该主机便可以加入该 AP 所属的子网。',
    ],
    start: 1,
  },
  {
    ...
    simple: "",
    normal: [
      '代理发现：指的是当某个移动 Ip 站点到达一个新网络时，不管是连接到一个外部网络还是返回其归属网络，它都必须知道相应的外部代理或归属代理的身份，这是新外部代理的发现，通过一个新的网络地址，才使移动节点中的网络层知道它已进入一个新的外部网络。',
      '注册过程：当收到一个外部代理的通告后，移动节点立即向外部代理发送一个移动 IP注册报文，外部代理收到注册报文并记录移动节点的永久 IP 地址，',
      '归属代理接受注册请求并检查其真实性和正确性，把移动节点的永久 IP 地址和 COA 绑定, 外部代理接收注册应答，然后将其转发给移动节点，到此注册就完成了，移动节点就能收到发往其永久地址的数据报了。'
    ],
    start: 1,
  },
