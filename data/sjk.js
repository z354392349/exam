...
{
    simple: "第二章",
    title: "简述数据库数据完整性的含义。 / 注意这个是数据库数据",
    normal: [
...
{
    title: "什么是数据库的完整性? / 注意这个是数据库 和上面的题容易混",
    simple: "第二章",
    normal: [
...

{
    ...
    normal: [
        ...
        '什么是良好的数据库性能（挂在这里容易记）：主要是指对数据的高效率存取和 空间的节省，并具有良好的数据共享性、完整性、一致性及安全保密性。',
...
{
    title: "数据库设计的过程",
    simple: "第三章 / 请简述数据库应用软件设计与实现的基本步骤 容易混    ",
    normal: [
...

{
    ...
    title: "简述索引的定义 / 不熟练",
    normal: [
        '索引是 DBMS 根据表中的一列或若干列 按照一定顺序建立的 列值与记录行之间的对应关系表，',
        '因而索引实质上是一张描述索引列的列值与原表中记录行之间一一对应关系的有序表。'
    ],
    start: 1,
},
...
},
{
    ...
    title: "什么是唯一性索引？",
    normal: [
        '唯一性索引和普通索引基本相同，只是有一点区别，即唯一性索引列中的所有值都只能出现一次，必须是唯一的。',
        '创建唯一性索引时，使用的关键字 UNIQUE ',
    ],
    start: 1,
},
{
    ...
    title: "简述普通索引的含义",
    normal: [
        '普通索引是最基本的索引类型，没有任何限制。创建普通索引时通常使用的关键字 INDEX或 KEY。',
    ],
    start: 1,
},
{
...
// 91
{
    simple: "第四章/ 过滤类型，是否可以包含聚合函数，过滤的时间",
...
// 91
{
    ...
    title: "请简述数据备份与恢复的定义",
    normal: [
        '数据库备份是指通过 导出数据或者复制表文件的方式 来制作数据库的复本；',
...
{
    simple: "第五章",
    title: "请简述局部变量与用户变量之间的区别 / @符号，作用域",
    normal: [
        '两者的区别是：局部变量声明时，在其前面没有使用@符号，并且它只能在声明它的 BEGIN„END 语句块中的语句所使用；',
        '而用户变量在声明时，会在其名称前面使用@符号，同时已声明的用户变量存在于整个会话之中。',
    ],
    start: 1,
},
{
    simple: "第五章 / 增良高减安 / 不熟练",
...
},
{
    ...
    title: "简述事务与程序的关系",
    normal: [
        '事务与程序很相似，但它们是两个彼此相联而又不同的概念：程序是静止的，事务是动态的，是程序的执行而不是程序本身；',
        '同一程序的多个独立执行可以同时进行，每一步执行则是一个不同的事务。',
    ]
},
{
    simple: "第六章",
...
{
    simple: "第六章",
    title: "请简述数据完整性约束的含义 / 易忘",
    normal: [
        '数据完整性约束是为了防止数据库中 存在不符合语义的数据，',
        '为了维护数据的完整性，DBMS 必须提供一种机制来检查数据库中的数据，以判断其是否满足语义规定的条件。这些加在数据库数据之上的语义约束条件就是数据完整性约束。',
    ],
    start: 1,
},
...

{
    ...
    simple: "第八章",
    normal: [
        '完成系统的实现工作之后，在正式交付用户使用之前，需要对所开发的系统进行必要的测试，验证其是否满足用户的功能要求，',
